...
public class Backtracking2 {
    ...
    public static void generateCombination(int[][] matrix, int index, int[] combination, int[] bestCombination,
        ...
        if (index == matrix.length) { // If the index is equal to the length of the matrix
            ...
            Measurement.incrementComparisons(1);
            if (fitness > getFitness(bestCombination, matrix)) { // If the fitness is greater than the fitness of the
                                                                 // best combination
        ...
        for (int i = 0; i < totalOfProfessors; i++) { // Iterate over the professors
            ...
            if (professors[i] < 4 && matrix[index][i] != -1) { // If the professor has less than 4 courses and the grade
                                                               // is not -1
                Measurement.incrementAssignments(4);
                combination[index] = i; // Set the professor to the combination
                professors[i]++; // Increase the total of courses of the professor

                generateCombination(matrix, index + 1, combination, bestCombination, professors); // Generate the
                                                                                                  // combination
